    ...
    public class NativeHookDemo
    {
        ...
        public void Main()
        {
            ...
            if (Environment.Is64BitProcess)
            {
                ...
                nativeHook = HookPool.GetInstance().AddNativeHook(new NativeMethod("MessageBoxA", "user32.dll"), nativeReplacementMethod, false);
            }
            else
            {
        ...
        private static int MessageBoxReplacement64Bits(IntPtr hwnd, IntPtr textPtr, IntPtr bodyPtr, uint type)
        {
            ...
            NativeHook nativeHook = HookPool.GetInstance().ReturnNativeHook("user32.dll", "MessageBoxA");
            if (nativeHook != null)
                return (int)nativeHook.CallOriginalMethod<MessageBoxDelegate, object>(hwnd, "Hooked (original: " + texte + ")", titre, type);
            return -1;
        }
...
#pragma warning restore IDE0060, S1172
        {
            ...
            NativeHook nativeHook = HookPool.GetInstance().ReturnNativeHook("user32.dll", "MessageBoxA");
            if (nativeHook != null)
                return (int)nativeHook.CallOriginalMethod<MessageBoxDelegate, object>(hwnd, "Hooked (original: " + text + ")", title, type);
            return -1;
        }
    }
}
